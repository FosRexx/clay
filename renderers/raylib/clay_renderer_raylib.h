...
Ray GetScreenToWorldPointWithZDistance(
  Vector2 position, Camera camera, int screenWidth, int screenHeight,
  float zDistance
);
    ...
    switch (renderCommand->commandType) {
      ...
      case CLAY_RENDER_COMMAND_TYPE_BORDER: {
        ...
        if (config->right.width > 0) {
          DrawRectangle(
            (int
            ) roundf(boundingBox.x + boundingBox.width - config->right.width),
        ...
        if (config->bottom.width > 0) {
          DrawRectangle(
            ...
            (int
            ) roundf(boundingBox.y + boundingBox.height - config->bottom.width),
            (int) roundf(
